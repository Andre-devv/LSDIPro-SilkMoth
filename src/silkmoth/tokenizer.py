...
def qgram_tokenize(input_set: list, q: int = 3) -> list[set[str]]:
    ...
    # new qchunk_tokenize (only small fix from previous qgram_tokenize):
def qchunk_tokenize(input_set: list, q: int = 3) -> list[set[str]]:
    """
    Tokenizes the input using non-overlapping q-chunks (required for edit similarity).

    Args:
        input_set (list): The input set (e.g., a record with strings or nested values)
        q (int): The q-chunk length (default: 3)

    Returns:
        list[set[str]]: A list of sets, each containing non-overlapping q-chunk tokens
    """
    def to_qchunks(s: str) -> set[str]:
        s = s.strip()
        if len(s) < q:
            return {s}
        return {s[i:i+q] for i in range(0, len(s) - q + 1, q)} #add before return here condition before if len(s) >= q??

    tokens = []
    for element in input_set:
        if isinstance(element, (str, int, float, bool)):
            tokens.append(to_qchunks(str(element)))
        elif isinstance(element, (list, tuple)):
            sub_tokens = set()
            for sub_element in element:
                if isinstance(sub_element, (str, int, float, bool)):
                    sub_tokens.update(to_qchunks(str(sub_element)))
                elif isinstance(sub_element, (list, tuple)):
                    for sub_sub_element in sub_element:
                        if isinstance(sub_sub_element, (str, int, float, bool)):
                            sub_tokens.update(to_qchunks(str(sub_sub_element)))
                        else:
                            raise ValueError(
                                f"Unsupported nested type: {type(sub_sub_element)}"
                            )
...
class Tokenizer:
    ...
    def tokenize(self, input_set: list) -> list:
        ...
        if self.sim_func == jaccard_similarity:
            ...
            tokens = qchunk_tokenize(input_set) #non overlapping 
        else:
