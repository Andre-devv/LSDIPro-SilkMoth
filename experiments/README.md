...
> 💡 *Current performance improvements use `scipy`’s C-accelerated matching, replacing the initial `networkx`-based implementation.*
...
> Similar to Schema Matching, but uses a GitHub WebTable as a fixed reference set  
> and matches it against other sets. The aim was to test SilkMoth’s behavior across different domains.
